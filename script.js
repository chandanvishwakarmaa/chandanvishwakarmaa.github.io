...
/* 9. CONTACT FORM DISPATCH & REAL TRANSMISSION */
window.handleFormSubmit = function(event) {
  ...
  // Look for any Web3Forms access key hidden input
  const accessKeyInput = form.querySelector('input[name="access_key"]');
  const accessKey = accessKeyInput ? accessKeyInput.value : "";

  const originalBtnText = submitBtn.innerHTML;
  submitBtn.innerHTML = 'Formatting transmission... <span class="animate-pulse">●</span>';
  submitBtn.disabled = true;

  // Use real Web3Forms payload submission if an active access key is found
  if (accessKey && accessKey !== "YOUR_ACCESS_KEY_HERE" && accessKey.trim() !== "") {
    submitBtn.innerHTML = 'Uploading payload... <span class="animate-pulse">●</span>';
    ...
    formData.append("subject", "Portfolio Message from " + nameVal);
    ...
    .then(async (response) => {
      ...
      if (result.success) {
        successAlert.innerHTML = '<span class="icon-success">&check;</span> Message processed successfully! Routed directly to Chandan\'s inbox via Web3Forms.';
    ...
    .catch((err) => {
      console.error(err);
      successAlert.innerHTML = `⚠️ Transmission failed: ${err.message}. Opening email client backend fallback instead...`;
      successAlert.classList.remove("hidden");
      
      setTimeout(() => {
        draftMailto(nameVal, emailVal, messageVal);
      }, 1500);
    })
    .finally(() => {
      setTimeout(() => {
    ...
    // Elegant fallback execution with a descriptive guide modal
    setTimeout(() => {
      successAlert.innerHTML = '<span class="icon-success">&check;</span> Message compiled! Opening your email client to dispatch securely...';
      successAlert.classList.remove("hidden");
      
      // Auto-draft mailto immediately
      draftMailto(nameVal, emailVal, messageVal);
      
      // Show elegant dialog helping user/owner configure Web3Forms in 10 seconds
      showWeb3FormsDeploymentAdvise(nameVal, emailVal, messageVal);
...
// Helper: Open secure prefilled mail client transition
function draftMailto(name, email, message) {
  const recipient = "chandanvishwakarma998797@gmail.com";
...
// Show advice modal for how to activate direct email alerts in production
function showWeb3FormsDeploymentAdvise(name, email, message) {
  const modal = document.getElementById("details-modal");
  const modalPath = document.getElementById("modal-path-text");
  const modalBody = document.getElementById("modal-dynamic-body");

  if (!modal || !modalBody || !modalPath) return;

  modalPath.innerText = "static-form-routing / deployment-guide.md";
  
  modalBody.innerHTML = `
    <div style="text-align: left; padding: 10px 5px;">
      <h2 style="font-family: var(--font-display, inherit); font-size: 1.6rem; font-weight: 700; margin-bottom: 0.75rem; color: #fff; display: flex; align-items: center; gap: 8px;">
        📬 Static Form Routing Support
      </h2>
      <p style="font-size: 0.9rem; line-height: 1.5; color: #ccc; margin-bottom: 1.5rem;">
        Your portfolio is designed to be fully static, allowing for instant hosting on <strong>GitHub Pages</strong>. Because there is no private backend server running, contact messages run on clientfallbacks or free email-routing services.
      </p>

      <div class="glassmorphism" style="border: 1px solid rgba(255,255,255,0.08); padding: 1.25rem; border-radius: 12px; margin-bottom: 1.5rem; background: rgba(255, 255, 255, 0.02);">
        <h4 style="font-family: var(--font-mono, monospace); font-size: 0.8rem; color: #82aaff; text-transform: uppercase; margin-bottom: 0.5rem; font-weight: 600; display: flex; align-items: center; gap: 6px;">
          <span>⚡</span> INSTANT ACTIONS TAKEN
        </h4>
        <p style="font-size: 0.85rem; color: #bbb; line-height: 1.4; margin: 0;">
          Your browser has launched your default email client with a fully drafted copy of your message ready to send to <strong>chandanvishwakarma998797@gmail.com</strong>.
        </p>
      </div>

      <h3 style="font-family: var(--font-display, inherit); font-size: 1.1rem; font-weight: 600; margin-bottom: 0.5rem; color: #fff;">
        ⚙️ Activating 100% Free Background Email Delivery
      </h3>
      <p style="font-size: 0.85rem; color: #aaa; margin-bottom: 1rem; line-height: 1.4;">
        To let future recruiters send emails right from your website without opening an external mail client, follow these simple, free steps:
      </p>

      <ol style="padding-left: 1.25rem; font-size: 0.85rem; color: #ccc; line-height: 1.6; margin-bottom: 1.5rem;">
        <li style="margin-bottom: 0.5rem;">
          Go to <a href="https://web3forms.com" target="_blank" style="color: #82aaff; text-decoration: underline;">https://web3forms.com</a> and input your receiver email (<strong>chandanvishwakarma998797@gmail.com</strong>). Within seconds, they'll email you a free Access Token.
        </li>
        <li style="margin-bottom: 0.5rem;">
          Open your local <strong><code>index.html</code></strong> file.
        </li>
        <li style="margin-bottom: 0.5rem;">
          Find your contact form and locate this input tag:<br/>
          <code style="display: block; background: rgba(0,0,0,0.3); padding: 8px; border-radius: 6px; margin: 4px 0; font-family: monospace; font-size: 0.78rem; border: 1px solid rgba(255,255,255,0.05); color: #82aaff; overflow-x: auto;">&lt;input type="hidden" name="access_key" value="YOUR_ACCESS_KEY_HERE"&gt;</code>
          Replace <code>"YOUR_ACCESS_KEY_HERE"</code> with safety tokens supplied by Web3Forms.
        </li>
        <li style="margin-bottom: 0.5rem;">
          Deploy to GitHub. Now, prospective recruiters will transmit messages in the background automatically!
        </li>
      </ol>

      <div style="display: flex; gap: 0.75rem; justify-content: flex-end; margin-top: 1.5rem;">
        <button class="btn btn-secondary text-xs" onclick="closeProjectModal()" style="border-radius: 30px; padding: 10px 20px;">
          Got it, proceed
        </button>
      </div>
    </div>
  ...
  // Display the modal using transition triggers
  modal.classList.remove("hidden");
  setTimeout(() => modal.classList.add("visible"), 50);
